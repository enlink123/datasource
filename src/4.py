...
xl = xlrd.open_workbook('/Users/mac/PycharmProjects/茂盛数据爬虫代码/src/基本信息ID.xls')
table = xl.sheets()[0]

for i in range(0, 30821):
    rowData = table.cell(i ,0).value
    print(rowData)
    soup = BeautifulSoup(get_html("http://hngcjs.hnjs.gov.cn" + rowData, 'lxml'))
    divContent = soup.findAll(name='div',attrs={"class":"news_con"})
    tds = divContent[0].select("td")
    index = 0
    for tdIndex, td in enumerate(tds):

        if tdIndex != 0 :
            if tdIndex % 2 ==0:
                spanstr = td.find("span")
                str = ""
                if spanstr is None:
                    str = ""
                else:
                    str = spanstr.string
                worksheet.write(i, index, str)
                index = index+1
    workbook.save("公司基本信息.xls")