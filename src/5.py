...
for i in range(0, 30821):
    ...
    soup = BeautifulSoup(get_html("http://hngcjs.hnjs.gov.cn/company/Qyzz?corpcode=" + rowData, 'lxml'))
    ...
    for sIndex, span in enumerate(spans):
        if sIndex == 0:
            originStr = originStr + span.string
            originStr = originStr + '&'
        else:
            if span.string in ('建筑业企业资质', '工程设计', '工程勘察', '工程监理', '招标代理机构', '检测机构', '安全生产许可证'):
                originStr = originStr + '$'
                originStr = originStr + span.string
                originStr = originStr + '&'
            else:
    ...
    #print(originStr)
    listArr = originStr.split("$")
    for lIndex, lStr in enumerate(listArr):
        worksheet.write(index+lIndex, 0, rowData)
        arr = lStr.split("&")

        endStr = ''
        for aIndex, aStr in enumerate(arr):
            if aStr.endswith(','):
                endStr = endStr + aStr
            else:
                worksheet.write(index+lIndex, aIndex+1, aStr)
        worksheet.write(index+lIndex, 6, endStr)
    index = index + len(listArr)
    workbook.save("6.xls")